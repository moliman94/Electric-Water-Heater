...
# 1 "./Temp_Sens_Task.h" 1
# 13 "./Temp_Sens_Task.h"
# 1 "./STATES.h" 1
# 13 "./Temp_Sens_Task.h" 2
...
void main(void)
{
    ...
    if(EEPROM_Read(0x80) != _1st_RUNTIME_CHECK_PATTERN)
    {
        for(DelayCount = (((8000000UL)/1000)*1/152); DelayCount>0; DelayCount--) __asm("NOP");;
        EEPROM_Write(0x00, 60);
        for(DelayCount = (((8000000UL)/1000)*1/152); DelayCount>0; DelayCount--) __asm("NOP");;
        EEPROM_Write(0x80, _1st_RUNTIME_CHECK_PATTERN);
    }
    for(DelayCount = (((8000000UL)/1000)*1/152); DelayCount>0; DelayCount--) __asm("NOP");;
    Set_temp = EEPROM_Read(0x00);
    for(DelayCount = (((8000000UL)/1000)*1/152); DelayCount>0; DelayCount--) __asm("NOP");;
